      ...
      {HERO_SLIDES.map((slide, idx) => (
        ...
        >
          ...
          <div className="relative h-full container mx-auto px-6 flex flex-col justify-center items-center md:items-start text-center md:text-left z-20">
            ...
            <p className={`text-lg md:text-2xl text-white/60 max-w-2xl mb-4 font-light leading-relaxed transition-all duration-1000 delay-[1200ms] ${idx === current ? 'opacity-100 translate-x-0' : 'opacity-0 translate-x-10'}`}>
              {t(`hero.slides.${idx}.subtitle`)}
            </p>

            {/* Tagline de segmentación */}
            <p className={`text-[10px] md:text-[11px] font-bold uppercase tracking-[0.35em] text-nexo-lime/60 mb-10 transition-all duration-1000 delay-[1600ms] ${idx === current ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-4'}`}>
