    ...
    return (
        <div className="group cursor-pointer">
            ...
            <div className="flex items-center gap-4 pt-4 border-t border-white/5 opacity-40 group-hover:opacity-100 transition-all duration-500 translate-y-2 group-hover:translate-y-0">
                ...
                <a href={`https://wa.me/?text=${encodeURIComponent(`${t('whatsapp.share_project')}${project.title} - https://nexofilm.com/portfolio/${project.id}`)}`} target="_blank" className="w-8 h-8 flex items-center justify-center rounded-full bg-white/5 hover:bg-nexo-lime hover:text-black transition-all">
                ...
                <a href={`https://www.linkedin.com/sharing/share-offsite/?url=${encodeURIComponent(`https://nexofilm.com/portfolio/${project.id}`)}`} target="_blank" className="w-8 h-8 flex items-center justify-center rounded-full bg-white/5 hover:bg-white hover:text-black transition-all">
                    <svg className="w-3.5 h-3.5 fill-current" viewBox="0 0 24 24"><path d="M19 0h-14c-2.761 0-5 2.239-5 5v14c0 2.761 2.239 5 5 5h14c2.762 0 5-2.239 5-5v-14c0-2.761-2.238-5-5-5zm-11 19h-3v-11h3v11zm-1.5-12.268c-.966 0-1.75-.79-1.75-1.764s.784-1.764 1.75-1.764 1.75.79 1.75 1.764-.783 1.764-1.75 1.764zm13.5 12.268h-3v-5.604c0-3.368-4-3.113-4 0v5.604h-3v-11h3v1.765c1.396-2.586 7-2.777 7 2.476v6.759z" /></svg>
                </a>
                <button onClick={(e) => { e.stopPropagation(); onCopyLink(`https://nexofilm.com/portfolio/${project.id}`, project.id); }} className="relative w-8 h-8 flex items-center justify-center rounded-full bg-white/5 hover:bg-nexo-lime hover:text-black transition-all">
